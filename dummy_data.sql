-- First, let's insert the dummy athletes and capture their new IDs
...
-- Now, insert some sample activities for these newly created athletes!
...
SELECT 
    'act_v1_' || id, id, 'Morning 5K Run', 'Run', NOW() - INTERVAL '1 day', 5.0, 1500, 1550, 45.0 
FROM inserted_athletes WHERE name = 'Vineet'
UNION ALL
SELECT 
    'act_v2_' || id, id, 'Weekend Long Run', 'Run', NOW() - INTERVAL '3 days', 10.0, 3000, 3100, 100.0 
FROM inserted_athletes WHERE name = 'Vineet'
UNION ALL
SELECT 
    'act_b1_' || id, id, 'Evening Cycle', 'Ride', NOW() - INTERVAL '2 days', 20.0, 3600, 3700, 150.0 
